...
class UNet(nn.Module):  # UNet AttU_Net
    def __init__(self, img_ch=7, output_ch=1):
        ...
        self.Maxpool5 = nn.MaxPool2d(kernel_size=2, stride=2)  ############# 2
        self.Maxpool6 = nn.MaxPool2d(kernel_size=4, stride=4)  ############# 4
        self.Maxpool7 = nn.MaxPool2d(kernel_size=8, stride=8)  ############# 8
    ...
    def forward(self, x):
        ...
        # 直接让其看
        d2_outline = single_channel.repeat(1, 64, 1, 1)
        ...
        d5 = self.Up5(x5)
        x4_att = self.Att5(g=d5_outline, x=x4)
        x4 = x4 + x4_att
        d5 = torch.cat((x4, d5), dim=1)
        d5 = self.Up_conv5(d5)

        d4 = self.Up4(d5)
        x3_att = self.Att4(g=d4_outline, x=x3)
        x3 = x3 + x3_att
        d4 = torch.cat((x3, d4), dim=1)
        d4 = self.Up_conv4(d4)

        d3 = self.Up3(d4)
        x2_att = self.Att3(g=d3_outline, x=x2)
        x2 = x2_att + x2
        d3 = torch.cat((x2, d3), dim=1)
        d3 = self.Up_conv3(d3)

        d2 = self.Up2(d3)
        x1_att = self.Att2(g=d2_outline, x=x1)
        x1 = x1_att + x1
