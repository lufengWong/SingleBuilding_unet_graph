...
class UNet(nn.Module):  # UNet AttU_Net
    def __init__(self, img_ch=7, output_ch=1):
        ...
        self.Con1_4 = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=1)
        self.Con1_5 = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=1)
        self.Con1_6 = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=1)
        self.Con1_7 = nn.Conv2d(in_channels=2, out_channels=1, kernel_size=1)
    ...
    def forward(self, x):
        ...
        d2_avg_out = torch.mean(x, dim=1, keepdim=True)
        d2_max_out = torch.max(x, dim=1, keepdim=True)[0]
        d2_outline = torch.cat((d2_avg_out, d2_max_out), dim=1)
        d2_outline = self.Con1_4(d2_outline)

        d3_outline_max = self.Maxpool5(x)
        d3_outline_avg = self.AveragePool5(x)
        d3_avg_out = torch.mean(d3_outline_avg, dim=1, keepdim=True)
        d3_max_out = torch.max(d3_outline_max, dim=1, keepdim=True)[0]
        d3_outline = torch.cat((d3_avg_out, d3_max_out), dim=1)
        d3_outline = self.Con1_5(d3_outline)


        d4_outline_max = self.Maxpool6(x)
        d4_outline_avg = self.AveragePool6(x)
        d4_avg_out = torch.mean(d4_outline_avg, dim=1, keepdim=True)
        d4_max_out = torch.max(d4_outline_max, dim=1, keepdim=True)[0]
        d4_outline = torch.cat((d4_avg_out, d4_max_out), dim=1)
        d4_outline = self.Con1_6(d4_outline)

        d5_outline_max = self.Maxpool7(x)
        d5_outline_avg = self.AveragePool7(x)
        d5_avg_out = torch.mean(d5_outline_avg, dim=1, keepdim=True)
        d5_max_out = torch.max(d5_outline_max, dim=1, keepdim=True)[0]
        d5_outline = torch.cat((d5_avg_out, d5_max_out), dim=1)
