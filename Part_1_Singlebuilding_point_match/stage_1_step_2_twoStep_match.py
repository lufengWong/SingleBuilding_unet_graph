...
def get_graph_location(points_ploygon_input, apartments_need, num_elevators, paths_png, paths_graph):
    ...
    # fine tune
    # 提取 中心坐标 提取图结构
    dict_id_graph_location= {}
    for index, (id, feature_chose) in enumerate(zip(list_ids_now, list_channel_1_plus_3_rot)):

        graph = dict_id_graph[id]
        # 处理一下图数据
        graph_clear = [pair.tolist() for pair in graph if (pair[0] != pair[1] and pair[0] < 300 and pair[1] < 300)]
        graph = []
        for pair in graph_clear:
            if [pair[1], pair[0]] not in graph:
                graph.append(pair)

        list_mask_less_300 = [mask for mask in list(set(list(feature_chose.ravel()))) if mask < 300]
        dict_mask_center = {value: compute_centroid(feature_chose, value) for value in list_mask_less_300}

        dict_id_graph_location.update( {id: [graph, dict_mask_center]})

    # 最后选中的户型旋转角度的字典(根据面积的交并比) 最后选中户型对应的图结构以及节点的位置(没有旋转) 所有户型对应的特征图 所有户型对应的图结构
    return list_ids_now, dict_id_rot, dict_id_graph_location, dict_id_png, dict_id_graph
...
if __name__ == '__main__':
    ...
    list_ids_now, dict_id_rot, dict_id_graph_location, dict_id_png, dict_id_graph = \
